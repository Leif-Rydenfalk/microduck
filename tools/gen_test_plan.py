...
<b>No EEPROM writes are prescribed by SV-01.</b> Original variant and reviewed configuration remain unresolved. Torque OFF alone does not establish permission, correct settings or a safe supply.</p>
<div class="tw"><table class="data reg"><caption>Table %d. Local M288-oriented register reference; preserve original settings until identity review.</caption>
<thead><tr><th>Addr</th><th>Bytes</th><th>Name</th><th>Area</th><th>Vendor initial</th><th>Local reference (no write authorization)</th><th>Meaning</th><th>Why</th></tr></thead>
<tbody>%s</tbody></table></div>
...
# ---- the refusals ---------------------------------------------------------
CHECKS = [
 ("unresolved original power procedures and their EOL rows remain HOLD",
  "runtime voltage mapping cannot authorize an overvoltage test on standard XL330 hardware"),
...
def selfcheck(doc=None):
    ...
    from testplan_power_guard import check as check_power_hold
    bad = check_power_hold(D)
...
<div class="resolved"><h4>HOLD — original power configuration unresolved</h4>
<p>{E(D['power_execution_hold']['reason'])}</p>
<p>{E(D['power_execution_hold']['release_condition'])}</p>
<p>The {n_tests} procedures and {len(D["eol"])} EOL checklist entries are definitions, not completed tests. Held entries cannot be ticked PASS.</p></div>
