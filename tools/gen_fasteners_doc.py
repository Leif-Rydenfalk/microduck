...
AUDIT = load("out/fasteners/audit.json")
...
def audit_rows():
    A = AUDIT["counts"]["by_check"]
    names = sorted({k.rsplit(" / ", 1)[0] for k in A})
    out = []
    for n in names:
        p_ = A.get("%s / PASS" % n, 0)
        f_ = A.get("%s / FAIL" % n, 0)
        c_ = A.get("%s / CANNOT DETERMINE" % n, 0)
        ex = ""
        for r in AUDIT["runs"]:
            for c in r["checks"]:
                if c["check"] == n:
            ...
            if ex:
                break
        out.append('<tr><td>%s</td><td>%s</td><td class="n">%d</td><td class="n">%d</td>'
                   '<td class="n">%d</td></tr>' % (E(n), E(ex), p_, f_, c_))
    for r in AUDIT["assembly_wide"]:
        out.append('<tr><td>%s <span class="dim">(assembly-wide)</span></td><td>%s</td>'
                   '<td class="n">%s</td><td class="n">%s</td><td class="n">%s</td></tr>'
                   % (E(r["check"]), E(r["what"]),
                      1 if r["verdict"] == "PASS" else 0,
                      1 if r["verdict"] == "FAIL" else 0,
                      1 if r["verdict"] == "CANNOT DETERMINE" else 0))
    return "\n".join(out)


def finding_rows():
    out = []
    for it in AUDIT["runs_with_a_finding"]:
        for c in it["checks"]:
            if c["verdict"] == "PASS":
                continue
            out.append('<tr><td>%s&nbsp;&times;&nbsp;%g</td><td><code>%s</code></td><td>%s</td>'
                       '<td>%s</td><td><code>%s</code></td><td>%s</td></tr>'
                       % (E(it["size"]), it["length_mm"], E("/".join(it["meshes"])),
                          E(c["check"]), chip(c["verdict"]),
                          E(json.dumps(c["measured"]))[:520], E(c.get("settled_by", ""))))
    return "\n".join(out) or '<tr><td colspan="6">no finding</td></tr>'
...
<div class="statbar">
  ...
  <div class="stat"><b>{AUDIT['counts']['per_run_checks'] + 2}</b><span>physical checks, {sum(v for k,v in AUDIT['counts']['by_check'].items() if k.endswith('FAIL'))} FAIL</span></div>
...
<nav class="toc">
  ...
  <a href="#audit">4 Does it work?</a><a href="#refused">5 What is not determined</a>
  <a href="#ball">6 Ball joints</a><a href="#parts">7 The screw parts</a><a href="#method">8 Method</a>
</nav>
...
<section id="audit">
  <h2><span class="n">4</span>Does this fastening actually work? {AUDIT['counts']['per_run_checks'] + 2} physical checks</h2>
  <div class="verdict"><b>{AUDIT['counts']['runs_all_PASS']} of {AUDIT['counts']['runs_audited']}
  runs pass every check; {AUDIT['counts']['runs_with_a_FAIL']} FAIL and
  {AUDIT['counts']['runs_with_a_CANNOT_DETERMINE']} CANNOT DETERMINE.</b>
  {E(AUDIT['framing'])}</div>
  <div class="tw"><table class="data">
    <caption>Table 3. Eight questions, asked of every run. Source:
      <code>out/fasteners/audit.json</code>, generated by
      <code>tools/fastener_audit.py</code>.</caption>
    <thead><tr><th>Check</th><th>What it asks</th><th class="n">PASS</th><th class="n">FAIL</th>
      <th class="n">CANNOT&nbsp;DETERMINE</th></tr></thead>
    <tbody>
{audit_rows()}
    </tbody></table></div>
  <div class="tw"><table class="data">
    <caption>Table 4. Every row that is not a PASS, with the number behind it and what settles
      it.</caption>
    <thead><tr><th>Screw</th><th>Where</th><th>Check</th><th>Verdict</th><th>Measured</th>
      <th>What settles it</th></tr></thead>
    <tbody>
{finding_rows()}
    </tbody></table></div>
  <p class="note"><b>Two readings were caught by these checks and both changed the model.</b>
  (1) Four runs <code>bearing_roll</code>&nbsp;&rarr;&nbsp;<code>yaw2roll</code> came out
  M2.5&nbsp;&times;&nbsp;5 because the per-hole classifier&rsquo;s labels were settled by a
  majority vote &mdash; one &Oslash;2.05 pilot outvoting one &Oslash;2.2 clearance &mdash; and an
  M2.5 shank is &Oslash;2.50 and cannot pass a &Oslash;2.2 hole. The size is now DERIVED from the
  holes: the shank must clear the tightest clearance on the chain and exceed the pilot it bites.
  Ten chains now have <b>no screw that fits their own geometry</b> and are CANNOT DETERMINE
  &mdash; most likely those blind features are pin seats, not pilots.
  (2) The one remaining open row is a &Oslash;3.788&nbsp;mm bore against a head of
  d<sub>k</sub>&nbsp;max 3.800&nbsp;mm &mdash; a 0.012&nbsp;mm interference measured on a patch
  whose own least-squares residual is 0.0316&nbsp;mm. The instrument cannot separate the two, so
  it is open rather than a FAIL, and a caliper closes it. At d<sub>k</sub> min (3.62&nbsp;mm) the
  same bore clears by 0.168&nbsp;mm.</p>
</section>

<section id="refused">
  <h2><span class="n">5</span>What is <i>not</i> determined — {n_refused} chains</h2>
  ...
  <div class="tw"><table class="data">
    <caption>Table 5. Chains that produced no fastener, by reason.</caption>
...
<section id="ball">
  <h2><span class="n">6</span>Ball joints — the search, and its answer</h2>
  ...
  <div class="tw"><table class="data">
    <caption>Table 6. The {len(BALLS['near_misses_refuted'])} pairs that came within
...
{near_rows()}
  ...
  <div class="tw"><table class="data">
    <caption>Table 7. Every distinct spherical patch fitted on placed geometry ({n_sph} distinct
...
<section id="parts">
  <h2><span class="n">7</span>The screw parts — built parametrically, measured against ISO 4762</h2>
  ...
  <div class="tw"><table class="data">
    <caption>Table 8. Source: <code>out/fasteners/screw-parts-verify.json</code>, generated by
...
<section id="method">
  <h2><span class="n">8</span>Method — the whole chain, in order</h2>
  <ol class="lede">
