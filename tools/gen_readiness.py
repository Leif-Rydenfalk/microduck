...
row("assembly", "manual", "Assembly sequence — MANUAL.md + PLAYBOOK stations", "NOT_YET",
    ...
    "A historical nominal 64-screw placement schedule exists. Current run generation withdraws all 52 servo ISO-length proposals after the manufacturer 3 mm maximum/M2 tapping callout correction; rebuilding refuses to silently delete them. Original thread identity, qualified stack lengths and torque remain unverified. The nominal buy list omits %d modeled screws by size; this historical size comparison is not a purchase instruction. ToF/speaker/mic attachment and physical harness routes remain unresolved." % _fast_reconciliation["missing_nominal_sizes_qty"],
