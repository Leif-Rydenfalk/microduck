...
SECTIONS = [
    ("Start here", "What is this, is it real, and can a factory build it today.", [
        ("FACTORY-PACK.html", "Factory pack \u2014 the document a factory builds from",
         "The single coherent pack for a partner factory and its engineers, bilingual EN/zh: what the product is with the measured envelope and the sliced mass, then WHAT IS NOT READY at the front with the measurement behind each row and who closes it, then what to build \u2014 every drawing sheet with its verdict, every printed part with its real ce-slice grams and seconds and its licence, every bought line with a price read off a live page, the three custom boards with their true DRC state, the harness, the 11 assembly stations \u2014 then the 42 end-of-line acceptance gates, the licence position, and where every underlying file sits. It states once and plainly that no physical unit has ever been built or measured and that every dynamic number is simulation. Generated by tools/gen_factory_pack.py."),
        ...
        ("out/factory/workplan.json", "Work breakdown (data)",
         "The 25 parcels, the ten engineer tracks with their day totals, and the critical path."),
