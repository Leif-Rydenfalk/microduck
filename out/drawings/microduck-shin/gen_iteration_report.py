...
supplement=''
sp=p/'detail-A-supplement-build'/'result.json'
if sp.exists():
 s=json.loads(sp.read_text()); dc=s.get('detail_context') or {}; lr=s.get('radius_locator_readback',{})
 context=dc.get('context_readback',[])
 status='PASS' if len(context)==4 and all(c.get('verdict')=='PASS' for c in context) else 'PENDING'
 section=s.get('section_locator_readback') or {}
 section_note=('E061 now has an independently verified local Section D leader; complete physical feature coverage is not implied. E009/E047 remain pending.' if section.get('verdict')=='PASS' and section.get('source_id')=='R061' else 'E061 still needs its literal leader.')
 supplement=f'''<h2>Staged ankle supplement</h2><p>BUILD {esc(s['build_verdict'])}; eight gates {esc(s['sheet_verdict'])}; live-solid context recheck {status}. Context and detail scales are independently read back as {esc(s['render_scale_readback']['paper_scales'])}. Full contract remains {esc(s['full_contract_verdict'])}.</p><p>{esc(lr.get('printed_count'))} individual leaders and {esc(lr.get('coordinate_count'))} coordinate locators cover {esc(lr.get('scope_count'))} selected circular edges. {esc(section_note)} True top/bottom remain on the principal sheet; supplement interpretation is disclosed, not claimed as full acceptance.</p><p><a href="detail-A-supplement-build/microduck-shin.pdf">Staged supplementary PDF</a> · <a href="detail-A-supplement-build/result.json">Supplementary result</a> · <a href="detail-A-supplement-build/detail-source.json">Live source and camera evidence</a></p>'''
...
ce-cad/bin/cad tools/test_rendered_drawing.py
ce-cad/bin/cad tools/test_radius_locators.py</pre>
