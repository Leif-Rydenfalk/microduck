...
def build(doc, params=None):
    ...
    # 2. The inner cavity follows the R4 outer return with an R3 inner
    # return. A constant 1 mm outline inset cuts through the outer return
    # near x=69.5 and creates the long slit visible in the previous STL.
    # Keep the original outside envelope, back face and connector axes.
    inner_radius = R_EDGE - T_WALL
    cavity = []
    for k in range(10):
        th = math.radians(90.0 * k / 9)
        x = X_RIM + inner_radius * math.sin(th)
        inset = R_EDGE - inner_radius * math.cos(th)
        cavity.append((_outline(x, inset), x))
    first, second = cavity[0], cavity[1]
    fraction = 0.2 / (second[1] - first[1])
    before = [(a[0] + (a[0] - b[0]) * fraction,
               a[1] + (a[1] - b[1]) * fraction)
              for a, b in zip(first[0], second[0])]
    from cecad.indexed_loft import indexed_loft
    p.shape = p.shape.cut(indexed_loft([(before, X_RIM - 0.2)] + cavity, axis="x"))
