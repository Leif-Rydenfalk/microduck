...
**CANNOT DETERMINE** — host `part:radxa-zero-3w`, 22 part(s), 26 net(s), 76 finding(s).
...
| `hat` | `part:microduck-robot-hat-pcb` | soldered | GND, VBAT, UART2_TX, UART2_RX, I2C3_SDA, I2C3_SCL, I2S3_SCLK, I2S3_LRCK, I2S3_SDO, I2S3_SDI, V3V3_IN, V5_OUT, SERVO_V, DXL_DATA, J5_3V3, SPK+, SPK-, MIC_IN, MICBIAS, V1V8_OUT |
...
## 19 finding(s) that are not PASS

### CANNOT DETERMINE — `current/J5_3V3` · typical draw

```
1 of 2 part(s) drawing J5_3V3 have no sourced typical current:
  hat@direct             (no chip named) NOT SOURCED
      Total board load and Radxa header capacity not established.
sourced so far:
  tof@direct             vl53l5cx              95 mA
That comes to 95 mA, which is a FLOOR and NOT the total. The missing parts
...
```
1 of 2 part(s) drawing J5_3V3 have no sourced peak current:
  hat@direct             (no chip named) NOT SOURCED
      Total board load and Radxa header capacity not established.
sourced so far:
  tof@direct             vl53l5cx             150 mA
That comes to 150 mA, which is a FLOOR and NOT the total. The missing parts
are not drawing zero — nobody wrote down what they draw. Adding them in as 0 mA
is how a budget passes a net that browns out on the bench.
To close it: fetch the datasheet and add the part to cecad/data/chips.json, or
measure it and record what you found.
...
### CANNOT DETERMINE — `i2c` · hat@direct

```
microduck-robot-hat-pcb declares a I2C endpoint and no address is recorded for it.
You cannot talk to a chip whose address nobody wrote down — and on a net this one
shares with 1 other device(s), an unknown address also cannot be shown NOT to
collide.
```

> an address is not recorded and no document states one

— parts[].requires[].address
