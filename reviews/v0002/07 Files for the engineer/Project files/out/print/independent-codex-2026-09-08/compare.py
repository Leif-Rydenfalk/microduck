...
source=Path(__file__).with_name('analyze.py').read_text()
...
fig.suptitle('FAIL: exact mesh section (orange dashed) versus actual model extrusion (blue), support grey\nEye-ring annulus omitted; bottom-shell layers 95 and 98 contain supports only');fig.tight_layout();fig.savefig(OUT/'mesh-versus-extrusion-FAIL.png',dpi=180)
