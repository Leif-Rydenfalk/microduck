...
base=Path(__file__).parent;out=base.parent/'plates/HEAD-V8-candidate';doc=config.load();mc=copy.deepcopy(next(m for m in doc['printers'].values() if 'H2D' in m.get('model','')))
...
res=partgate.gate([str(out/'microduck-head-v8-bracket-flat.3mf')],mc,doc.get('policy',{}),mc['model'],'PLA',nozzle_pair=['0.4','0.6'],keep_dir=str(out));(out/'partgate-result.json').write_text(json.dumps(res,indent=2))
