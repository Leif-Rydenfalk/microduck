...
HISTORICAL ENVELOPE PROVENANCE. The initial geometric values were READ OFF Pollen's
...
SOURCE CORRECTION CANDIDATE 2026-09-08. Horn/idler pilot depth and the four
stepped case holes now follow the archived ROBOTIS PDF/STEP. Other exterior
geometry remains a simplified mesh-derived representation and original unit
identity, material and production fits remain unverified. Vendor frame maps
to this frame as local(X,Y,Z)=(vendorZ+8, vendorX, vendorY). See this folder's
vendor-geometry.json and the manufacturer mechanical-source audit.
...
# ---- drill ------------------------------------------------------------------
THRU_D = 1.6  # ROBOTIS STEP: central pilot, vendor Z -15..0 (15 mm)
CASE_FRONT_CLEAR_D, CASE_FRONT_CLEAR_DEPTH = 2.0, 3.5
CASE_REAR_CLEAR_D, CASE_REAR_CLEAR_DEPTH = 2.0, 4.5
THRU_Y = (-8.0, 8.0)         #   at y = ±8
THRU_Z = (7.5, -22.5)        #   at z = 7.5 and -22.5, i.e. body-centre z = -7.5 ± 15
FACE_D, FACE_DEPTH = 1.6, 2.9  # ROBOTIS STEP cylindrical spans; PDF specifies DP3.0(Max.)
FACE_ENGAGEMENT_MAX = 3.0  # manufacturer drawing maximum, not an exact thread profile
FACE_PCD = 12.0  # manufacturer PDF: Using M2 Tapping Screw; metric pitch is not inferred
...
def build(doc, params=None):
    ...
    # Four source-backed stepped case pilots; legacy mesh omitted these steps.
    for y in THRU_Y:
        for z in THRU_Z:
            p.cyl(THRU_D, 40, at=(-20, y, z), axis="x", op="cut")
            p.cyl(CASE_FRONT_CLEAR_D, CASE_FRONT_CLEAR_DEPTH + 0.01,
                  at=(BODY_X[1] - CASE_FRONT_CLEAR_DEPTH, y, z), axis="x", op="cut")
            p.cyl(CASE_REAR_CLEAR_D, CASE_REAR_CLEAR_DEPTH + 0.01,
                  at=(BODY_X[0] - 0.01, y, z), axis="x", op="cut")
    # Eight manufacturer STEP Ø1.6 x 2.9 mm cylindrical pilots; 3 mm drawing maximum.
    for y, z in FACE_AT:
