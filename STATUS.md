...
| rung | verdict | proof |
|---|---|---|
| 1 · every part on the shelf | **DONE BY DECISION** (2026-09-02, Leif: 'use it directly') — Pollen's meshes are the primary geometry for every part; 20 parametric rebuilds PASS and replace their slugs; the remaining organic shells stay mesh-backed on purpose. Previously: **PARTIAL** — 54 slugs; **15 parametric rebuilds PASS** vs Pollen's meshes (worst p95 1.00 mm — shin; best 0.001 — trunk-base; ankles 0.012, hip-bracket 0.162/0.242, thighs 0.20, feet/neck done in round 2), ~30 mesh-backed vendor parts build as loaders; head shells, face, jaw group + connection folders still queued (limit killed them twice) | `tools/watch-pass.log`, `out/refcheck/*/`, per-folder `evidence/` |
