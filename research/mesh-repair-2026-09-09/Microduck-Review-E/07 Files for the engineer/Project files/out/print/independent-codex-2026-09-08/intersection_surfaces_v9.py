...
source=Path(__file__).with_name('analyze_v9.py').read_text();exec(source.split('# Independent software')[0]);parser=source[source.index('# Parse actual motion'):source.index("print('Parsed'")];full=z.read('Metadata/plate_1.gcode').decode().split('; CHANGE_LAYER');fig,axes=plt.subplots(2,2,figsize=(12,10));rows=[]
