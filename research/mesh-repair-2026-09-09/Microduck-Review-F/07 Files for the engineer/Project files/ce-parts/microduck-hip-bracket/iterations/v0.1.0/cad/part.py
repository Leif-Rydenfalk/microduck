...
def _circular(center, radius, a0, a1):
    def point(angle):
        a = math.radians(angle)
        return (center[0] + radius*math.cos(a), center[1] + radius*math.sin(a))
    return ("arc", point((a0+a1)/2), point(a1))


def _profile(p, start, steps, depth, at, axis, op="add"):
    """Exact lines/circular arcs; do not fillet a polygon approximation."""
    import FreeCAD as App
    import Part as K
    def vector(q):
        u, v = q
        if axis == "x": return App.Vector(at[0],at[1]+u,at[2]+v)
        if axis == "y": return App.Vector(at[0]+u,at[1],at[2]+v)
        return App.Vector(at[0]+u,at[1]+v,at[2])
    current=vector(start);first=current;edges=[]
    for step in steps:
        if len(step)==3 and step[0]=="arc":
            middle,end=vector(step[1]),vector(step[2])
            edges.append(K.Arc(current,middle,end).toShape())
        else:
            end=vector(step)
            if (end-current).Length>1e-8: edges.append(K.makeLine(current,end))
        current=end
    if (current-first).Length>1e-8: edges.append(K.makeLine(current,first))
    normal={"x":(depth,0,0),"y":(0,depth,0),"z":(0,0,depth)}[axis]
    solid=K.Face(K.Wire(edges)).extrude(App.Vector(*normal))
    p._apply(solid,op)
...
def _guarded_fillet(p, r, where, label):
    ...
    p.clean()
    before = p.shape.copy()
    v0, b0 = p.shape.Volume, p.shape.optimalBoundingBox()
    try:
        ...
        return
    # OCCT BoundBox includes B-spline control poles outside the actual fillet.
    # Exact surface bounds preserve the original strict envelope guard.
    b = p.shape.optimalBoundingBox()
...
def build(doc, params=None):
    ...
    _profile(p,(X_ROLL-RIB_END_R,Y_BACK),[
        (RIB_CORNER_C[0],Y_BACK),_circular(RIB_CORNER_C,RIB_CORNER_R,-90,0),
        (X_PLATE_FRONT,RND_OUT_C[1]),(X_PLATE_BACK,RND_OUT_C[1]),
        (X_PLATE_BACK,Y_PLATE_TOP),(X_ROLL-RIB_END_R,Y_PLATE_TOP)],
        2*RIB_HW,(0,0,-RIB_HW),"z")
    _profile(p,(X_ROLL,-RIB_HW-0.1),[
        (X_ROLL,-RIB_END_R),_circular((X_ROLL,0),RIB_END_R,-90,-270),
        (X_ROLL,RIB_HW+0.1),(X_ROLL-RIB_END_R-2,RIB_HW+0.1),
        (X_ROLL-RIB_END_R-2,-RIB_HW-0.1)],
        Y_PLATE_TOP-Y_BACK+0.2,(0,Y_BACK-0.1,0),"y",op="cut")

    # The measured optional R1 outer rib round is intentionally omitted.
    # OCCT's valid-looking fillet crosses the roll counterbore surfaces:
    # 9 self-intersections alone, 7 with R2, 41 when applied after drilling.
    # Keep the actual sharp edge until a non-intersecting round is constructed.
    p.notes.append("R1 outer rib edge round omitted: intersecting fillet rejected; mounting features and R2 root rounds retained")

    # 3. THIN ROLL PLATE: stadium in (z, x), extruded along y from -21.5 by 2.5
    _profile(p,(RND_OUT_C[0],-R_END),[
        (X_ROLL,-R_END),_circular((X_ROLL,0),R_END,-90,-270),
        (RND_OUT_C[0],R_END)],Y_PLATE_TOP-Y_PLATE_BOT,(0,Y_PLATE_BOT,0),"y")

    # 4. CORNER: one x-y polygon = outer R8 quarter-round minus the inner R6
    #    fillet circle, full height z +/-9.5
    yi=RND_OUT_C[1]
    xi=FIL_IN_C[0]+math.sqrt(FIL_IN_R**2-(yi-FIL_IN_C[1])**2)
    a_in=math.degrees(math.atan2(yi-FIL_IN_C[1],xi-FIL_IN_C[0]))
    _profile(p,(RND_OUT_C[0],Y_PLATE_BOT),[
        _circular(RND_OUT_C,RND_OUT_R,-90,0),(xi,yi),
        _circular(FIL_IN_C,FIL_IN_R,a_in,-90),
        (RND_OUT_C[0],Y_PLATE_TOP)],W,(0,0,-HZ),"z")
    ...
    _profile(p,(RND_OUT_C[1],-R_END),[
        (Y_PITCH,-R_END),_circular((Y_PITCH,0),R_END,-90,90),
        (RND_OUT_C[1],R_END)],X_PLATE_FRONT-X_PLATE_BACK,(X_PLATE_BACK,0,0),"x")
