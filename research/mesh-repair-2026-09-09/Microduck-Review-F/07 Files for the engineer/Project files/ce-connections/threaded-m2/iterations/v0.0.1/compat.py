"""connection:threaded-m2 -- is this pair of M2 interfaces actually matable?
...
Shaped after ce-connections/threaded-m3 (the workshop's worked example for a
threaded joint; its own header says the arithmetic is deliberately duplicated
into each folder -- TRIAD.md line 51, an iteration folder is COMPLETE, never a
diff). The numbers are the M2 row of the ISO tables, DERIVED here from the
ISO 68-1 basic profile rather than recalled, and checked against two
independent statements of the same row:

  - the published M2 tensile stress area, 2.07 mm2 (the figure ISO 898-1
    tables carry for M2; quoted, not used in the derivation), and
  - ce-cad/cecad/fasteners.py line 57, the M2 row of the ISO 273 / ISO 4762
    table this workshop already builds against:
      ("M2", 2.0, 0.40, 1.60, 2.20, 2.40, 2.60, 3.8, 2.0, 4.4, 4.0, 5.0, 0.3)
    (name, d, coarse pitch, tap drill, close/normal/loose clearance, head_d,
    head_h, cbore_d, hex_af, washer_od, washer_t).

WHY M2 AND NOT ANOTHER SIZE: the Microduck is an M2 system, measured off
Pollen's own meshes -- SPEC.md section 4: "the community hole analysis reads
the whole robot as an M2 system (Ø2.2 clearance x77, Ø4.4 c'bore x28, Ø1.6
tap x20, Ø2.7/2.8 x20)", and the microduck ce-parts shelf declares 60+
interfaces accepting connection:threaded-m2 with exactly those diameters.
Ø2.2 is the fasteners.py M2 close clearance, Ø4.4 its M2 counterbore and
Ø1.6 its M2 tap drill -- three independent hole populations on the meshes
landing on the same table row.
...
PITCH_MM = 0.4          # ISO 261 coarse pitch for nominal diameter 2 mm
                        # (cross-check: cecad/fasteners.py M2 row, pitch 0.40)
NOMINAL_MM = 2.0
...
# (duplicated from ce-connections/threaded-m3, same defect history applies)
# --------------------------------------------------------------------------
...
def read_thread(iface, what="interface"):
    ...
    spelling "M2x0.4" (ce-parts writes both; the string crash is documented in
    ...
    if not isinstance(th, str):
        raise ValueError(
            ...
            "string spelling 'M2x0.4'. Nothing else is interpreted."
...
def thread_geometry(d_mm=NOMINAL_MM, p_mm=PITCH_MM):
    ...
    would show up as a disagreement with the published As (2.07 mm2 for M2).
    """
...
def preload_from_torque(torque_Nm, mu_thread, mu_bearing, d_km_mm,
    ...
    0004); nobody in this workshop has measured one for an M2 either.
    """
...
def compatible(a_iface, b_iface):
    ...
    if names == ("thread_ext", "thread_int"):
        ...
        add("interface_names", "FAIL", "a=%r, b=%r" % names,
            "connection:threaded-m2 mates thread_ext (external) to thread_int "
            "(internal), in that order -- the same pilot names threaded-m3 fixed.")

    # 2. both sides declare a thread, and it is M2
    geo = thread_geometry()
    for side, iface in (("a", a_iface), ("b", b_iface)):
        ...
        elif _norm_designation(des) in ("M2", "M2X0.4"):
            add("%s_thread_designation" % side, "PASS", des)
        else:
            add("%s_thread_designation" % side, "FAIL", des,
                "this folder is M2 coarse only; another size is another folder")
    ...
    if pa is None or pb is None:
        add("pitch_match", "CANNOT DETERMINE", {"a": pa, "b": pb},
            ...
            "the other. M2 x 0.25 fine exists and will not mate with M2 x 0.4.")
        ...
        add("pitch_match", "FAIL", {"a": pa, "b": pb},
            "both are %s mm, which is not the ISO 261 coarse pitch %s mm for M2"
            % (pa, PITCH_MM))
    else:
    ...
    if depth is None or a_len is None:
        add("engagement", "CANNOT DETERMINE",
            ...
            "usually a SELF-TAPPED PLA pilot (Ø1.55-1.65 measured on the meshes) "
    ...
    #    The M2 tap drill for ~65-70%% engagement is 1.60 mm
    #    (cecad/fasteners.py M2 row). Pollen's printed pilots measure
    #    Ø1.55..1.65 on the meshes -- inside a sensible band. A pilot at or
    #    above the 2.0 major has no material to cut a thread in: FAIL.
    pilot = _field(b_iface, "pilot_d_mm", "tap_d_mm")
    if pilot is not None:
        p = float(pilot)
        if p >= NOMINAL_MM:
            add("pilot_diameter", "FAIL", {"pilot_d_mm": p},
                "pilot >= the 2.0 mm major diameter: the screw has nothing to "
                "thread into")
    ...
    if worst == "PASS":
        why = ("all %d checks PASS. M2 x %s, ISO 68-1 profile, As = %.4f mm2 "
    ...
    return {"verdict": worst, "why": why, "checks": checks,
            ...
            "connection": "connection:threaded-m2"}


if __name__ == "__main__":
    g = thread_geometry()
    print(json.dumps(g, indent=2))
    published_as = 2.07     # the ISO 898-1 M2 figure this derivation is CHECKED AGAINST
    err = abs(g["As_tensile_stress_area_mm2"] - published_as)
    print("As derived = %.6f mm2; published M2 tensile stress area = %.2f mm2; "
          "delta = %.6f mm2 -> %s"
          % (g["As_tensile_stress_area_mm2"], published_as, err,
             "PASS" if err < 0.005 else "FAIL"))
    # published ISO 261/724 basic diameters for M2 x 0.4, quoted as the check:
    published = {"d2": 1.740, "d3": 1.509, "D1": 1.567}
