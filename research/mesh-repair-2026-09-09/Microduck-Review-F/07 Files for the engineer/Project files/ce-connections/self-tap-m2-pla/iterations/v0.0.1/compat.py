"""connection:self-tap-m2-pla -- is this pair actually matable, and is the pilot a
pilot?
...
WHAT THIS FILE GRADES THAT connection:threaded-m2's CANNOT
-----------------------------------------------------------------
threaded-m2 grades a CUT thread against ISO fits. There is no cut thread here.
What can be graded is:
  ...
  2. the pilot DIAMETER, against the band MEASURED on the reference meshes:
     33 pilots of this size, 23 of them at exactly Ø1.6 mm
  ...
  5. the HOLDING -- and this one is always CANNOT DETERMINE, on purpose. No
     M2 screw has been pulled out of a printed Ø1.6 pilot in this
...
PILOTS_MEASURED_MM = [1.471, 1.477, 1.477, 1.48, 1.48, 1.482, 1.489, 1.499, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.6, 1.636, 1.636]
...
def compatible(a_iface, b_iface):
    ...
    if names == ("thread_ext", "pilot"):
        ...
        add("interface_names", "FAIL", "a=%r, b=%r" % names,
            "connection:self-tap-m2-pla mates thread_ext (the screw) to pilot (the PRINTED HOLE), "
            "in that order. 'thread_int' means a CUT thread and is connection:threaded-m2.")

    th = read_thread(a_iface, "a_iface")
    des = (th.get("designation") or "")
    if str(des).upper().replace(" ", "") in ("M2", "M2X0.4"):
        add("screw_size", "PASS", des)
    elif not des:
        add("screw_size", "CANNOT DETERMINE", None,
            "a_iface states no thread.designation; nothing is assumed")
    else:
        add("screw_size", "FAIL", des, "this folder is M2 only")
    ...
    add("holding", "CANNOT DETERMINE", None,
        ...
        "M2 screw thread-formed in FDM PLA at a Ø%.2f pilot. connection.json "
