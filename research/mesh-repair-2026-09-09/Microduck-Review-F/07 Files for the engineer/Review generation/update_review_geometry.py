...
def refresh(out):
 ...
 # Thin head shells must use their full source triangulation in the assembly.
 # The catalog preview had reduced them from 16120/20970 to 7292/4482 faces.
 for row in rows:
  slug=row['slug']
  if slug not in ('microduck-top-head-shell','microduck-bottom-head-shell'):continue
  src=out/row['stl'];m=stl_mesh(src);vertices=np.array(m['positions']).reshape(-1,3)
  for i,place in enumerate(placements):
   if place['part']!='part:'+slug:continue
   name=place['body']+'/'+slug+'#'+str(i);part=next(p for p in assembly['parts'] if p['name']==name)
   before=len(part['indices'])//3
   w,x,y,z=np.array(place['world_quat_wxyz'])/np.linalg.norm(place['world_quat_wxyz']);rot=np.array([[1-2*(y*y+z*z),2*(x*y-z*w),2*(x*z+y*w)],[2*(x*y+z*w),1-2*(x*x+z*z),2*(y*z-x*w)],[2*(x*z-y*w),2*(y*z+x*w),1-2*(x*x+y*y)]])
   world=vertices@rot.T+place['world_pos_mm'];part.update(positions=world.reshape(-1).tolist(),indices=list(range(len(vertices))),edges=[],tris=len(vertices)//3)
   proof.append({'part':slug,'source':row['stl'],'sha256':sha(src),'triangles':len(vertices)//3,'previous_preview_triangles':before,'change':'Full source shell replaces simplified display mesh; source STL unchanged'})
 csvwrite(out/'PARTS.csv',rows);csvwrite(out/'SOURCE FILES.csv',sources)
 assembly['review_revision']='Five current CAD exports and two full-resolution source shells placed through source MJCF frames';(out/'01 See the robot/Published assembly mesh.json').write_text(json.dumps(assembly,separators=(',',':')))
