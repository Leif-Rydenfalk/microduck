"""connection:press-fit-bearing-22x16x4 -- seat the 22x16x4 bearing on a boss or in a bore.
...
ce-connections/press-fit-608 and this shelf's press-fit-bearing-15x10x3
(the same joint one size down); the frame arithmetic is deliberately
...
(2026-09-02) reads hole Ø16.0 / boss Ø22.0, bbox 22.0 x 22.0 x 4.0 -- the
22 mm OD x 16 mm bore x 4 mm ring SPEC.md section 4 counts x11 on the robot:
the hip-yaw, hip-roll, hip-pitch, head-yaw and head-roll axes all turn on
it (spec/mesh-placements.json). No vendor designation exists in the
reference; the slug is the dimensions.

THE SEATS this joint was written against, each with its own measured
interface on the microduck shelf:
    inner ring on a boss   microduck-yaw2roll 'yaw_bearing_seat':
                           Ø16.0 x 1.95 boss with its own Ø19.0 x 0.5
                           shoulder; microduck-hip-bracket 'roll_/
                           pitch_bearing_seat': Ø16.0 x 1.95;
                           microduck-yaw-roll-motion 'roll_bearing_seat':
                           Ø16.0 x 4.0 with Ø12 through bore
    outer ring in a bore   the mating body's Ø22 pocket (trunk_base for
                           hip-yaw, the head for head-roll);
                           microduck-bearing-roll 'roll_bearing_face' is
                           the outer-face side of the hip-roll ring

For a press fit J is a pure translation along z: the two axes are COAXIAL
and CO-DIRECTED, and the only freedom is where along the axis the ring
lands. That is fixed by the shoulder it is pressed against; if no shoulder
is declared this module RAISES rather than seating the ring at 0.0 (the
press-fit-608 rule, kept verbatim).
...
BORE_MM = 16.0    # meshfeatures hole d on the reference bearing mesh
OD_MM = 22.0      # meshfeatures boss d, same run
WIDTH_MM = 4.0    # numpy bbox z 0..4 of the same mesh
...
def _declared_joint():
    ...
    if not isinstance(j, dict):
        raise ValueError("connection:press-fit-bearing-22x16x4: connection.json "
...
def _joint(dof_left, **over):
    ...
    for tok in dof_left:
        ...
        if ent is None:
            raise ValueError(
                "connection:press-fit-bearing-22x16x4 mate() returned dof_left "
...
def mate(a_iface, b_iface, params=None):
    """Seat the 22x16x4 (a_iface: 'bore' or 'od') onto a seat (b_iface).

    a_iface -- the BEARING side. Name 'bore' (Ø16 inner ring) or 'od' (Ø22
    ...
    b_iface -- the seat: 'shaft'/'boss' (mates 'bore') or 'housing_bore'/
               'pocket' (mates 'od'); the microduck shelf's roles
               'bearing_seat'/'bearing_face' with either name also serve.
               Needs frame, nominal_mm, and either shoulder_z_mm or
               params['seat_dz_mm'].
    ...
    if a_name not in BEARING_IFACES:
        raise ValueError("a_iface must be the bearing side; the interface names "
                         "are fixed as 'bore' (Ø16) and 'od' (Ø22), got %r"
                         % (a_name,))
    ...
    if b_name not in SHAFT_NAMES + HOUSING_NAMES and b_role not in (
        ...
        raise ValueError("b_iface must be the seat: 'shaft'/'boss' (mates "
                         ...
                         "role 'bearing_seat'/'bearing_face'; got name %r "
                         "role %r" % (b_name, b_role))
    ...
    for iface, side, aliases in (
        ...
        if abs(float(nom) - nominal) > 0.5:
            raise ValueError("%s nominal_mm is %r; connection:press-fit-bearing-"
                             "22x16x4 seats %s mm on the '%s' side. A 15x10x3 "
                             "seat is connection:press-fit-bearing-15x10x3."
    ...
    if params.get("seat_dz_mm") is not None:
        ...
        datum_source = ("b_iface['shoulder_z_mm'] -- the ring is pressed until "
                        "its seated face touches the shoulder (yaw2roll's seat "
                        "states its own Ø19.0 x 0.5 shoulder for exactly this)")
    else:
        raise ValueError(
            ...
            "has no self-locating feature along its axis.")
    ...
    if params.get("spans_bearing"):
        ...
        dof_why = ("The mated pair spans the bearing (params['spans_bearing']), "
                   ...
                   "that is the idler side of the hip-yaw, hip-roll, hip-pitch, "
                   "head-yaw and head-roll axes, where this ring sits opposite "
                   "the servo horn.")
    else:
        dof_left = []
        dof_why = ("ONE SEAT considered alone -- ring on boss, or ring in "
                   "bore -- and an interference fit is rigid: no DOF left. "
    ...
    if retention not in (None, "shoulder_both_sides", "clamped", "glued"):
        raise ValueError("params['axial_retention'] %r is not one of None, "
                         ...
                         "option: no circlip groove exists on any measured "
                         "microduck seat and no circlip folder is on this shelf "
                         "-- naming one would dangle." % (retention,))
        ...
        "why": {
            ...
            "ring": {"d_mm": BORE_MM, "D_mm": OD_MM, "B_mm": WIDTH_MM,
                     "cite": "cecad.meshfeatures + bbox on seeed_bearing__"
                             "configuration__22x16x4.stl, 2026-09-02"},
...
if __name__ == "__main__":
    fr = lambda o, z: {"origin_mm": list(o), "z_axis": list(z), "x_axis": [1, 0, 0]}
    ring = {"name": "bore", "nominal_mm": 16.0,
            "frame": fr((17.5, 17.5, 14.45), (0, 0, 1))}
    boss = {"name": "yaw_bearing_seat", "role": "bearing_seat",
            "nominal_mm": 16.0, "seat_length_mm": 1.95, "shoulder_z_mm": 0.0,
            "abutment_dia_mm": 19.0, "frame": fr((17.5, 17.5, 14.45), (0, 0, 1))}
    m = mate(ring, boss)
    print("dof_left:", m["dof_left"], "| adds:", m["adds"],
          "| holds_by:", m["joint"]["holds_by"])
    print("spanning:", mate(ring, boss, {"spans_bearing": True})["dof_left"])
    for label, a, b, p in (
            ("no shoulder and no seat_dz", ring,
             {k: v for k, v in boss.items() if k != "shoulder_z_mm"}, {}),
            ("a 10 mm ring's seat", ring, dict(boss, nominal_mm=10.0), {}),
            ("a circlip nobody stocks", ring, boss,
             {"axial_retention": "circlip_external"})):
        try:
            mate(a, b, p)
            print("NOT REFUSED (defect):", label)
        except ValueError as exc:
            print("refused %-28s %s" % (label + ":", str(exc)[:90]))
